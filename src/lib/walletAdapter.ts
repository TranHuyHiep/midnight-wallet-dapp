  ...
  const walletProvider: WalletProvider = {
    ...
    async balanceTx(
      ...
      try {
        ...
        console.log(`[WalletAdapter] balanceTx: as String first 128 chars: ${new TextDecoder().decode(tx.serialize()).slice(0,128)}]`)
        ...
        const deserializedTx = Transaction.deserialize('signature', 'pre-proof', 'pre-binding', resultBytes) as Transaction<SignatureEnabled, PreProof, PreBinding>
        ...
        const prover = await connectedAPI.getProvingProvider(zkConfigProvider.asKeyMaterialProvider());
